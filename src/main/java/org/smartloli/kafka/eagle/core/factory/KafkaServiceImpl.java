...
public class KafkaServiceImpl implements KafkaService {
	...
	public long getKafkaRealLogSize(String clusterAlias, String topic, int partitionid) {
		...
		TopicPartition tp = new TopicPartition(topic, partitionid);
		consumer.assign(Collections.singleton(tp));
		Map<TopicPartition, Long> endLogSize = consumer.endOffsets(Collections.singleton(tp));
		Map<TopicPartition, Long> startLogSize = consumer.beginningOffsets(Collections.singleton(tp));
		try {
	...
	public long getKafkaRealLogSize(String clusterAlias, String topic, Set<Integer> partitionids) {
		...
		consumer.assign(tps);
		Map<TopicPartition, Long> endLogSize = consumer.endOffsets(tps);
		Map<TopicPartition, Long> startLogSize = consumer.beginningOffsets(tps);
		try {
